...
RegisteredVariable makeIntRef(int* v){
    return (RegisteredVariable){
        .type = R_INT, 
        .ir_value = v
    };
...
bool string_to_int(std::string s, int * val){
    if(s.size() == 0) return false;
    try {
        *val = std::stoi(s);
        return true;
...
void handleValueInput(std::string input, RegisteredVariable& variable){
    ...
    switch(variable.type){
        ...
        case R_INT:{
            int * ref = variable.ir_value;
            int value;
            if(ref == NULL){ null_ref = true; break;}
            if(!string_to_int(input, &value)){ invalid_input = true; break; }
            *ref = value; input = std::to_string(value);
            break;
        }
        case R_STRING:{
...
std::string variableValueToString(RegisteredVariable r){
    switch(r.type) {
        ...
        case  R_STRING:
            ...
            return * r.sr_value;
        case  R_INT:
            log_assert(NULL != r.ir_value, "");
            return std::to_string(* r.ir_value);
    }
    return "???";
} 
